...
class Usuario:
    ...
    def obtener_todos(cls):
        ...
        try:
            usuarios = conexion.obtener("SELECT usu.id, usu.nombre, usu.email, usu.id_tipousuario, tu.nombre as tipousuario"
    ...
    def obtener_por_id(cls, usuario_id):
        conexion = bd.Conexion()
        try:
            usuario = conexion.obtener("SELECT usu.id, usu.nombre, usu.email, usu.id_tipousuario, tu.nombre as tipousuario"
    ...
    #REGISTRAR
    @classmethod
    def registrar(cls, nombre, email, password, idTipoUsuario, usuario):
        conexion = bd.Conexion()
        ...
        try:
            ...
            conexion.ejecutar("CALL SP_REGISTRAR_USUARIO(%s, %s, %s, %s, %s);", (nombre, email, password_hash, idTipoUsuario, usuario))
    ...
    #EDITAR
    @classmethod
    def editar(cls, id, nombre, email, idTipoUsuario):
        conexion = bd.Conexion()

        try:
            # Llamar al procedimiento almacenado
            conexion.ejecutar("CALL SP_EDITAR_USUARIO(%s, %s, %s, %s);", (id, nombre, email, idTipoUsuario))
